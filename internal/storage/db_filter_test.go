...
func TestListJobsVerdictFilter(t *testing.T) {
	...
	for i, jobType := range []string{JobTypeTask, JobTypeInsights, JobTypeFix} {
		job, enqueueErr := db.EnqueueJob(EnqueueOpts{
			RepoID: repo.ID, GitRef: fmt.Sprintf("verdict-unscored-%d", i),
	...
	openFailing, err := db.ListJobs("", "", 1, 0, WithVerdict(false), WithClosed(false))
	require.NoError(t, err)
